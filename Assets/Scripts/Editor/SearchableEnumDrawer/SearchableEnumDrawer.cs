...
namespace RoboRyanTron.SearchableEnum.Editor {
    ...
    public class SearchableEnumDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
			...
			float editButtonWidth = 0F;
			if (Enum.TryParse(property.enumNames[property.enumValueIndex], out ProjectEnums.LocID parsedID)) {
				if ((ProjectEnums.LocID)property.enumValueFlag == parsedID) {
					editButtonWidth = 35;
					GUIContent editLocIDButton = new GUIContent("Edit");
					if (GUI.Button(new Rect(position.x, position.y, editButtonWidth, EditorGUIUtility.singleLineHeight), editLocIDButton)) {
						EditLocIDUtility.Init(parsedID);
					}
				}
			}

            if (DropdownButton(id, new Rect(position.x + editButtonWidth, position.y, position.width, position.height), buttonText)) {
